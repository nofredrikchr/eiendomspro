...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') return res.status(200).json({ bygg: await listBygg(eierId) });
    if (req.method === 'POST') return res.status(201).json({ bygg: await opprettBygg(eierId, req.body ?? {}) });
